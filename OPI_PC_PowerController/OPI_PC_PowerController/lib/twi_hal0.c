...
uint8_t *twi_buff;
uint8_t byteIndex;
volatile  uint8_t twi_mode = 0;
volatile  uint8_t status = 0xF8; //volatile
volatile  uint8_t twiReady;
...
uint8_t twi0_read_noReg(uint8_t addr, uint8_t *data, uint16_t len){
	...
	TWDR0 = (addr << 1) | 1;  // Address + READ (R/W = 1)
	err = twi0_addr_read_ack();
	if (err != TWI_OK) {
		twi0_stop();
		return err;
