 ...
 *  @author Goce Boshkovski
 */
gpio sensorPin = {NULL , NULL};

uint8_t OWReset(TSDS18x20 *pDS18x20)
{
	uint8_t result;
	gpio_set_pin_level(&sensorPin, false);
	gpio_set_pin_direction(&sensorPin, PORT_DIR_OUT);
	_delay_us(480);
	
	gpio_set_pin_direction(&sensorPin, PORT_DIR_IN);
	_delay_us(80);
	result = gpio_get_pin_level(&sensorPin);
	_delay_us(400);
...
uint8_t OWReadBit(TSDS18x20 *pDS18x20)
{
	...
	gpio_set_pin_direction(&sensorPin, PORT_DIR_IN);
	_delay_us(8);
	if (gpio_get_pin_level(&sensorPin))
		result=1;
	_delay_us(48);
...
uint8_t DS18x20_Init(TSDS18x20 *pDS18x20,volatile uint8_t *DS18x20_PORT,uint8_t DS18x20_PIN)
{
	...
	sensorPin.port = DS18x20_PORT;
	sensorPin.pin = DS18x20_PIN;
