...
def serverUdpIncomingData(can_message):
    ...
    filename = f"0x{can_id:08X}.log"
    raw_data = ' '.join(f'{b:02X}' for b in data_bytes)

    if(can_id == 0x69):
        log_line = f"SensorUnit, dalas Temp:{can_message.data[0]}, boilerTemperature: {can_message.data[1]}, coolantTemperature: {can_message.data[2]}, solarCollectorTemp: {can_message.data[3]}"
        print(log_line)
        write_to_file(filename, f"[ {log_line} ] | RAW: [{raw_data}] DLC: {dlc}")
        
    elif(can_id == 0x33):
        log_line = f"UPS, Battery U: {data_bytes[0]/10} V; I: {data_bytes[1] << 8 | data_bytes[2]} mA; SOC: {data_bytes[3] << 24 | data_bytes[4] << 16 | data_bytes[5] << 8 | data_bytes[6]} mAh; Mode: {data_bytes[7]}"
        print(log_line)
        write_to_file(filename, f"[ {log_line} ] RAW: [{raw_data}] DLC: {dlc}")

    elif(can_id == 0x43):
        log_line = f"UPS, Pump State: {data_bytes[0]}, Mode: {data_bytes[7]}"
        print(log_line)
        write_to_file(filename, f"[ {log_line} ] RAW: [{raw_data}] DLC: {dlc}")
        
    elif(can_id == 0x247):
        log_line = f"Kotel, State: {data_bytes[0]}: ActTemp: {(data_bytes[1] << 8 | data_bytes[2])/100} C, HLimit: {data_bytes[3]}, Llimit: {data_bytes[4]}, RunTemp: {data_bytes[5]}, StopTemp: {data_bytes[6]}, DaltaT: {data_bytes[7]}"
        print(log_line)
        write_to_file(filename, f"[ {log_line} ] RAW: [{raw_data}] DLC: {dlc}")
    else:
        log_line = f"Unknown CAN ID: 0x{can_id:08X}, Data: [{raw_data}], DLC: {dlc}"
        print(log_line)
        write_to_file(filename, f"[ {log_line} ] RAW: [{raw_data}] DLC: {dlc}")

def write_to_file(filename, log_line):
    """
    Записує рядок в файл з timestamp
    
    Parameters:
    filename (str): Ім'я файлу
    log_line (str): Рядок для запису
    """
    if not writeFile:
        return
