...
void ISL29034_init(const uint8_t run_mode, const enum data_resolution_enum measure_range, const enum lux_resolution_enum lux_range){
	...
	twi_write(ISL29034_ADDR,COMMAND_1,reg_conf,sizeof(reg_conf));
}

uint16_t read_light_intensity(void){
	uint8_t reg_raw[2];
	twi_read(ISL29034_ADDR, DATA_LSB, reg_raw, sizeof(reg_raw));
	return (reg_raw[1] << 8) + reg_raw[0];
}
uint8_t read_id (void){
	uint8_t reg_raw[1];
	twi_read(ISL29034_ADDR, ISL29034_ID, reg_raw, sizeof(reg_raw));
	return reg_raw[0];
}